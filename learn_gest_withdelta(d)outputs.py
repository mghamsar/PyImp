...
def on_gui_change(x,s_index):
	try:
		...
		if (compute==0):
			...
			print ("Outputs: ", data_output)	

	except:
		print ("On Gui Change Error!")
		raise
...
for s_index in range(num_outputs):
	...
	sliders[s_index]=Tkinter.Scale(master,from_=0,to=100, label='output'+str(s_index+1),orient=Tkinter.HORIZONTAL,length=300,command=tc(s_index))
...
def calculate_output_derrivatives(mapper_output):
	print ("inputs",data_input.values())

	print ("outputs",data_output.values()) 
...
for l_num in range(num_inputs):
	l_inputs[l_num]=l_map.add_input("/input/"+str(l_num+int(10)),1,'f',None,0,100.0,h)
	l_map.poll(0)
	print ("creating input", "/input/"+str(l_num+int(10)))
	
#######create mapper signals (outputs)
for l_num in range(num_outputs):
	l_outputs[l_num]=l_map.add_output("/output/"+str(l_num),1,'f',None,0,1)
	l_outputs_derrivatives[l_num]=l_map.add_output("/output/derrivative/"+str(l_num),1,'f',None,0,1)
	l_map.poll(0)
